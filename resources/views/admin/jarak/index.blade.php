...
@section('page-title', 'School-to-Village Distance Matrix')
...
<div class="container-fluid px-0 mb-5">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <h4 class="mb-1 text-dark fw-bold">Distance Matrix Spasial</h4>
            <p class="text-muted small mb-0">Monitoring list of the real level of proximity from school institutions to the target village area.</p>
        </div>
    ...
    <div class="card modern-card shadow-sm">
        <div class="card-header bg-white py-3 border-0 rounded-top-4">
            <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                <h5 class="table-header-title fw-bold mb-0">Spatial Proximity List</h5>
                <div class="d-flex align-items-center gap-3">
                    ...
                    <div class="search-wrapper">
                        ...
                        <input type="text" class="search-input" id="tableSearch" placeholder="Search schools or villages..." autocomplete="off">
                    </div>
                </div>
            </div>
        </div>
                ...
                <table class="table custom-table table-hover align-middle mb-0" style="min-width: 1000px;">
                    <thead class="table-light text-secondary">
                        <tr>
                            ...
                            <th width="30%">ROI Wilayah / Desa</th>
                            <th width="20%">Real Distance</th>
                    ...
                    <tbody id="jarakTableBody">
                        ...
                        <tr class="searchable-row">
                            ...
                            <td>
                                <span class="fw-bold text-primary fs-6">{{ $j->jarak }} Km</span>
                            </td>
                            <td class="text-center pe-4">
                                <form action="{{ route('jarak.destroy', $j->id) }}" method="POST" onsubmit="return confirm('Delete data jarak ini?')" class="d-inline-flex gap-2 justify-content-center w-100">
                                    <a href="{{ route('jarak.show', $j->id) }}" class="btn btn-sm btn-action-view btn-icon" title="View">
                                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                    </a>
                                    <a href="{{ route('jarak.edit', $j->id) }}" class="btn btn-sm btn-action-outline btn-icon" title="Edit">
                        ...
                        <tr>
                            <td colspan="5" class="text-center py-5 text-muted small">
                                No distance data available. Please add new distance matrix data by clicking the "Add Distance Data" button above.
                            </td>
                        </tr>
                        @endforelse
...
(function () {
    ...
    const rows = Array.from(document.querySelectorAll('#jarakTableBody .searchable-row'));
