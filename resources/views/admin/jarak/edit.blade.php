...
@section('page-title', 'Change Matrix Distance Value')
        ...
        <div class="col-lg-8">
            ...
            <div class="d-flex justify-content-between align-items-center mb-4">
                <div>
                    ...
                    <p class="text-muted small mb-0">Change the parameters for the geographic proximity values of school institutions towards target village polygons.</p>
                </div>
            ...
            <div class="card modern-card shadow-sm">
                <div class="card-header bg-white py-4 border-0 rounded-top-4">
                    <h5 class="form-header-title fw-bold mb-0">Change Matrix Distance Value</h5>
                </div>
                
                <div class="card-body p-4 pt-2">
                    <form action="{{ route('jarak.update', $jarak->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        
                        <div class="mb-4">
                            <label for="sekolah_id" class="form-label form-label-custom mb-2">School Name</label>
                            <select class="form-select form-select-custom @error('sekolah_id') is-invalid @enderror" id="sekolah_id" name="sekolah_id" required>
                                @foreach($sekolahs as $s)
                                    <option value="{{ $s->id }}" {{ $jarak->sekolah_id == $s->id ? 'selected' : '' }}>
                                        {{ $s->nama_sekolah }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            <label for="wilayah_id" class="form-label form-label-custom mb-2">ROI Area / Desa</label>
                            <select class="form-select form-select-custom @error('wilayah_id') is-invalid @enderror" id="wilayah_id" name="wilayah_id" required>
                                @foreach($wilayahs as $w)
                                    <option value="{{ $w->id }}" {{ $jarak->wilayah_id == $w->id ? 'selected' : '' }}>
                                        {{ $w->nama_wilayah }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                <input type="number" step="any" class="form-control form-control-custom @error('jarak') is-invalid @enderror" id="jarak" name="jarak" value="{{ old('jarak', $jarak->jarak) }}" style="border-top-right-radius: 0; border-bottom-right-radius: 0;" required>
